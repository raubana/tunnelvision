...
function ENT:BeginPausing()
	...
	if not self.pausing then
		...
		self.pausing = true
		self.pausing_wants_to_stop = false
		self.pausing_end = CurTime() + Lerp( self.unstable_counter/Lerp(0.5, self.unstable_lower_hint_limit, self.unstable_upper_hint_limit), Lerp( math.random(), 30, 45 ), Lerp(math.random(), 0.5, 1) )
