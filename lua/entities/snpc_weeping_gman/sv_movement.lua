...
function ENT:UpdateRunOrWalk( len, no_pop )
	...
	if not self.target and ((len <= self.run_tolerance) or should_walk) then
...
function ENT:GiveMovingSpace( options )
	print( self, "GiveMovingSpace NEW" )
...
function ENT:MoveToPos( pos, options )
	print( self, "MoveToPos NEW" )
	
	while self.frozen and not self.interrupt do
		coroutine.yield()
	end
	...
	while self.path:IsValid() do
		if not self.frozen then
			...
			if self.path:GetAge() > ( options.repath or 0.1 ) then
				if self.target then
					pos = self.target_last_known_position
				end
